...
func signMultiSig(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType,
	addresses []ltcutil.Address, nRequired int, kdb KeyDB) ([]byte, bool) {
...
func sign(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	...
	ScriptClass, []ltcutil.Address, int, error) {
...
sigLoop:
	for _, sig := range possibleSigs {
		...
		for _, addr := range addresses {
			...
			// bad input.
			pkaddr := addr.(*ltcutil.AddressPubKey)
...
type KeyDB interface {
	GetKey(ltcutil.Address) (*btcec.PrivateKey, bool, error)
}
...
type ScriptDB interface {
	GetScript(ltcutil.Address) ([]byte, error)
}

// ScriptClosure implements ScriptDB with a closure.
type ScriptClosure func(ltcutil.Address) ([]byte, error)

// GetScript implements ScriptDB by returning the result of calling the closure.
func (sc ScriptClosure) GetScript(address ltcutil.Address) ([]byte, error) {
	return sc(address)
}
