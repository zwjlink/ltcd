...
// newAddressPubKey returns a new ltcutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) ltcutil.Address {
	addr, err := ltcutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new ltcutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) ltcutil.Address {
	addr, err := ltcutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new ltcutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) ltcutil.Address {
	addr, err := ltcutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the ltcutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the ltcutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := ltcutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := ltcutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := ltcutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := ltcutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := ltcutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*ltcutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*ltcutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*ltcutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*ltcutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*ltcutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
