...
// newAddressPubKey returns a new dashutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) dashutil.Address {
	addr, err := dashutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new dashutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) dashutil.Address {
	addr, err := dashutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new dashutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) dashutil.Address {
	addr, err := dashutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the dashutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the dashutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := dashutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := dashutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := dashutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := dashutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := dashutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*dashutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*dashutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*dashutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*dashutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*dashutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
